"""reset_dares_with_correct_csv_data

Revision ID: 875f2988022a
Revises: 622d5d598bc3
Create Date: 2025-11-27 13:48:18.879140
...
revision: str = '875f2988022a'
down_revision: Union[str, None] = '622d5d598bc3'
...
# Correct seed data for dares (from CSV with correct points)
DARES_DATA = [
    # Activity dares
...
def upgrade() -> None:
    # Delete all existing dare assignments (they reference old dares)
    op.execute("DELETE FROM daily_dare_assignments")

    # Delete all existing dares
    op.execute("DELETE FROM dares")

    # Re-insert dares with correct data from CSV
...
def downgrade() -> None:
    # This is a data migration - downgrade would need the old data which we don't have
    # Best we can do is clear the data
    op.execute("DELETE FROM daily_dare_assignments")
    op.execute("DELETE FROM dares")